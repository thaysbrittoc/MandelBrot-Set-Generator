// Mandelbrot
// Modified by Thays Costa(z5183075) on 2017-09-17
//
...
void routeRequest(int socket, char requestBuffer[REQUEST_BUFFER_SIZE]) {
    ...
    // assigning the variables for the coordinates
    int z;
    double x;
    double y; 

    //assigning the array to store the URL
    //and scanning the URL into the array.
    char address[256];
    sscanf(requestBuffer, "GET %s HTTP/", address);
    printf("address: %s\n", address);
    ...
    if (strcmp(address, "/") == 0) {
        ...
        sscanf(address,"/mandelbrot/2/%d/%lf/%lf/tile.bmp", &z, &x, &y);
        // parsed url values and store into x, y & z values
        printf("[x = %lf  y = %lf  z = %d]\n\n", x, y, z);
        // serve the requested BMP
